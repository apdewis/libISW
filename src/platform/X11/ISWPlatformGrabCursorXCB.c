...
/* X11 cursor font name→glyph mapping (cursorfont.h constants). */
static const struct { const char *name; unsigned int shape; } cursor_glyphs[] = {
...
static unsigned int
xcb_cur_name_to_shape(const char *name)
{
    for (unsigned int i = 0; i < sizeof(cursor_glyphs)/sizeof(cursor_glyphs[0]); i++)
        if (strcmp(name, cursor_glyphs[i].name) == 0)
            return cursor_glyphs[i].shape;
    return XC_X_cursor;
}

/* Theme-aware named cursor via xcb-cursor; glyph fallback. */
static IswCursor
xcb_cur_load_named(IswDisplay dpy, IswScreen screen, const char *name)
{
    xcb_connection_t *conn = _IswXcbConn(dpy);
    ...
    if (xcb_cursor_context_new(conn, scr, &ctx) < 0)
        return xcb_cur_create_glyph(dpy, xcb_cur_name_to_shape(name));
