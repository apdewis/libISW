...
static Boolean
drag_convert(Widget widget, xcb_atom_t target_type,
...
static void
drag_start_action(Widget w, xcb_generic_event_t *event,
                  String *params, Cardinal *num_params)
{
    ...
    /* xcb_motion_notify_event_t shares the core pointer-event layout with
       xcb_button_press_event_t (time, root/event, root_x/y, event_x/y), and
       ISWXdndStartDrag only reads those fields, so the trigger event can be
...
main(int argc, char *argv[])
{
    ...
    /* Add drag-start on Btn1 MOTION, leaving the IconView's own translations
       ...
       on motion BandDrag yields while ISWXdndIsDragging() and only runs when a
    ...
    /* Label's class does not declare the drop/drag callbacks as resources,
       so register them with the direct setters (per ISWXdnd.h). */
    ISWXdndWidgetAcceptDrops(drop_target);
    ISWXdndSetDropCallback(drop_target, drop_callback, NULL);
    ISWXdndSetDragMotionCallback(drop_target, drag_motion_callback, NULL);
    ISWXdndSetDragLeaveCallback(drop_target, drag_leave_callback, NULL);

    /* Only accept the types we know how to handle. */
    {
        xcb_atom_t accepted[2];
        accepted[0] = ISWXdndInternType(drop_target, "text/plain");
        accepted[1] = ISWXdndInternType(drop_target, "text/uri-list");
        ISWXdndSetAcceptedTypes(drop_target, accepted, 2);
        ISWXdndSetAcceptedActions(drop_target,
